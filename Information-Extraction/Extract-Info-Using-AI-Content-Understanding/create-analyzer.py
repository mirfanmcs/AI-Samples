...
def main():
    ...
    try:
        ...
        # Get config settings
        load_dotenv()
        ai_svc_endpoint = os.getenv('OPEN_AI_ENDPOINT')
        ai_svc_key = os.getenv('OPEN_AI_KEY')
        analyzer = os.getenv('ANALYZER_NAME')

        # Create the analyzer
        create_analyzer_using_rest (card_schema, analyzer, ai_svc_endpoint, ai_svc_key)
...
def create_analyzer_using_rest (schema, analyzer, endpoint, key):
    
    # Create a Content Understanding analyzer using REST API
